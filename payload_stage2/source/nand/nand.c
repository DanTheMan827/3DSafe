...
bool LoadKeyFromFile2(const char* aespath, const char* slotFilePath, u8* keydata, u32 keyslot, char type, char* id) {
	u8 key_magic[16];
    u8 buffer[32];
    u8* key = buffer + 16;
    bool found = false;
...
//     snprintf(path, 256, "%s/aeskeydb.bin", folder);
    ...
    for (u32 p = 0; FileGetData(aespath, buffer, 32, p) == 32; p += 32) {
...
//              (id) ? id : (type == 'X') ? "X" : (type == 'Y') ? "Y" : "");
        if (FileGetData(slotFilePath, key, 16, 0) == 16)
            found = true;
    }
...
{   
    ...
    if ((*(vu32*) 0x101401C0) == 0) { // only for a9lh
        ...
        */
        ...
        if ((memcmp(shasum, slot0x05KeyY_sha256, 32) != 0) && // last resort
            (!LoadKeyFromFile2("0:/aeskeydb.bin", "0:/Slot0x05KeyY.bin", slot0x05KeyY, 0x05, 'Y', NULL)) &&
            (!LoadKeyFromFile2("0:/Decrypt9/aeskeydb.bin", "0:/Decrypt9/Slot0x05KeyY.bin", slot0x05KeyY, 0x05, 'Y', NULL)) &&
            (!LoadKeyFromFile2("0:/files9/aeskeydb.bin", "0:/files9/Slot0x05KeyY.bin", slot0x05KeyY, 0x05, 'Y', NULL))) {
